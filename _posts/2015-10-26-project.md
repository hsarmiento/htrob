...
En lo que respecta al modelo de datos, estructurado mediante el **formato XML**, se puede encontrar en <a href="https://github.com/tchambil/rqt_env/blob/master/htbot.xml"> File XML</a>. El modelo describe las etiquetas padres `<robots>`  y `<general>` con sus respectivos etiquetas hijos. En sección `<robots>` cada robot tendrá un identificado y un estado atributo status que permitirá determinar si el Robot está activo o no (ejemplo, `<ROBOT ID="turtlebot" STATUS="1">`) de igual forma la sección `<general>` tendrá sus etiquetas hijo.
